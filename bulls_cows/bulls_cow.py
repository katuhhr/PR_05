...
• Записывайте все свои ходы и полученные результаты
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.start_background = QPixmap(
            "C:/Users/User/Desktop/bulls_cows/background.jpg"
        )
        self.game_background = QPixmap(
            "C:/Users/User/Desktop/bulls_cows/game_background.jpg"  # второй фон для игры
        )
        self.start_background = self.start_background.scaled(
            800, 500, Qt.AspectRatioMode.IgnoreAspectRatio
        )
        self.game_background = self.game_background.scaled(
            800, 500, Qt.AspectRatioMode.IgnoreAspectRatio
        )
        ...
        # Создаем поле для ввода (изначально скрыто)
    ...
    def generate_number(self):
        ...
        # Собираем число и сохраняем его
        self.secret_number = str(first) + "".join(map(str, rest))
        print(f"Загаданное число: {self.secret_number}")  # Для отладки
    ...
    def check_input(self, text):
        # Проверяем, что первая цифра не ноль
        ...
        # Если есть повторяющиеся цифры, оставляем только уникальные
    ...
    def check_number(self):
        ...
        # Обновляем текст с историей попыток (последние 8 попыток)
        ...
        # Проверяем на победу
        if bulls == 4:
            victory_text = f"Поздравляем! Вы угадали число {self.secret_number}!\nКоличество попыток: {len(self.attempts)}"
            self.result_label.setText(victory_text)
            self.input_field.setEnabled(False)  # Отключаем ввод
            self.check_button.setEnabled(False)  # Отключаем кнопку проверки
